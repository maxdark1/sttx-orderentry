...
export class ReporteComponent implements OnInit {
  ...
  public Usuario: User;
  public statusOV: string = "abierta";
  ...
  mostrarReporteCumplimiento() {
    ...
    this._ordenService.reporteCumplimiento(this.orden, this.site, this.region, this.vendido, this.vendedor, new Date(this.fecha1), new Date(this.fecha2), this.inside, this.embarcar).subscribe(
      response => {
        console.log(response);
        try {
    ...
    for (let i = 0; i < this.datos.length; i++) {
      ...
      if (this.statusOV == "abierta") {
        detalles = {
          ...
          "cantidad_ordenada": parseFloat(this.datos[i].cantidad_ordenada).toFixed(2),
          "cantidad_embarcada": parseFloat(this.datos[i].cantidad_embarcada).toFixed(2),
          "cantidad_pendiente": parseFloat(this.datos[i].cantidad_pendiente).toFixed(2),
        ...
        detalles = {
          ...
          "total_pt": parseFloat(this.datos[i].total_pt).toFixed(2),
          "pt_um": this.datos[i].pt_um,
          "fecha_embarque": new Date(this.datos[i].fecha_embarque),
        };
      }
  ...
  public exportarExcel() {
    this._ordenService.generarExcel(this.datos,this.statusOV).subscribe(
      response => {
        if (response = 200) {
  ...
  public rowCallback(context: RowClassArgs) {
    ...
    let fechaembarque = moment(context.dataItem.fecha_embarque);

    console.log();
    
    if (context.dataItem.fecha_embarque == "") {
      ...
      if (fechavencido >= fechaembarque){
        warning = false;
        danger = false;
        normal = true;
      }
      else if(fechavencido < fechaembarque){
        warning = false;
        danger = true;
        normal = false;
      }
